...
def test_sigmoid():
    ...
    assert pytest.approx(activations.sigmoid(jnp.array(1.0)), 0.0001) == 1 / (
        1 + jnp.exp(-1.0)
    )
    assert pytest.approx(activations.sigmoid(jnp.array(-1.0)), 0.0001) == 1 / (
        1 + jnp.exp(1.0)
...
def test_silu():
    ...
    # SiLU(1) = 1 * sigmoid(1)
    assert pytest.approx(activations.silu(jnp.array(1.0)), 0.0001) == float(
        1.0 * activations.sigmoid(jnp.array(1.0))
    )
...
def test_tanh():
    ...
    assert pytest.approx(activations.tanh(jnp.array(1.0)), 0.0001) == float(
        jnp.tanh(1.0)
    )
    assert pytest.approx(activations.tanh(jnp.array(-1.0)), 0.0001) == float(
        jnp.tanh(-1.0)
...
def test_softplus():
    ...
    assert pytest.approx(activations.softplus(jnp.array(1.0)), 0.0001) == np.log(
        1 + np.exp(1.0)
    )
